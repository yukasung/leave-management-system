...
export default function EditPositionForm({ position }: { position: PositionData }) {
  ...
  const [deleteState, setDeleteState] = useState<PositionFormState | null>(null)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border border-red-200 rounded-xl p-5 bg-red-50">
        ...
        {deleteState?.message && (
          <div className={`rounded-lg px-4 py-2 text-sm font-medium mb-3 ${deleteState.success ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-300'}`}>
            {deleteState.message}
          </div>
        )}

        {position._count.employees > 0 ? (
          <p className="text-xs text-red-500 font-medium">⚠️ ไม่สามารถลบได้ — มีพนักงาน {position._count.employees} คนที่ใช้ตำแหน่งนี้</p>
        ) : !deleteConfirm ? (
